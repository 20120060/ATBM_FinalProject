    ...
    public partial class listNhanVien : Form
    {
        ...
        private void btUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string[] listNameTC = new string[] { "MANV", "LUONG", "PHUCAP"};
                string[] listTextTC = new string[] { textMaNV.Text, textSalary.Text, textPhuCap.Text};

                if (this.conn.State == ConnectionState.Closed)
                    conn.Open();
                if (role == "TAICHINH")
                {
                    for (int i = 0; i < listTextTC.Length; i++)
                    {
                        if (this.conn.State == ConnectionState.Closed)
                            conn.Open();
                        if (listTextTC[i] == "" || listNameTC[i]=="MANV")
                            continue;
                        OracleCommand getEmps = conn.CreateCommand();
                        getEmps.CommandText = "OLS_TEST1.PROC_UPDATE_TAICHINH";
                        getEmps.CommandType = CommandType.StoredProcedure;
                        getEmps.Parameters.Add("COL", listNameTC[i]);
                        RSA rsa = new RSA(512);
                        byte[] buffer=new Byte[1024];
                        if (rsa.ImportPrivateKeyFromFile("../../keys/" + textMaNV.Text + ".xml") == 1)
                        {
                            if (listTextTC[i] != "")
                            {
                                //long bufferSize
                                //listText[i].GetBytes(6, 0, null, 0, 0); ;
                                buffer = Encoding.ASCII.GetBytes(listTextTC[i]);
                                //byte[] buffer = new byte[bufferSize];
                                //reader.GetBytes(6, 0, buffer, 0, (int)bufferSize);
                                

                                buffer = rsa.Encrypt(listTextTC[i]);
                            }
                            else
                            {
                                listTextTC[i] = "NULL";
                            }
                            //getEmps.Parameters.Add("VAL", rsa.Encrypt("1"));
                        }
                        else
                        {
                            rsa.ExportPrivateKeyToFile("../../keys/" + textMaNV.Text + ".xml");
                            rsa.ImportPrivateKeyFromFile("../../keys/" + textMaNV.Text + ".xml");
                        }
                        getEmps.Parameters.Add("VAL", rsa.Encrypt(listTextTC[i]));
        ...
        private void update_List_VaiTro()
        {
            ...
            OracleCommand getID = conn.CreateCommand();
            getID.CommandText = "select DISTINCT VAITRO from "+tab;
            ///conn.ConnectionString
            getID.CommandType = CommandType.Text;
            OracleDataReader reader = getID.ExecuteReader();
            while (reader.Read())
