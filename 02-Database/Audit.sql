...
(
    ...
    POLICY_NAME=>'CAU_C',
    AUDIT_COLUMN=>'LUONG,PHUCAP',
    AUDIT_CONDITION=>'UPPER(OLS_TEST1.F_VAITRO(USER)) != ''TAICHINH''',
    STATEMENT_TYPES=>'UPDATE',
    AUDIT_COLUMN_OPTS=>DBMS_FGA.ANY_COLUMNS
);
END;
...
SELECT * FROM UNIFIED_AUDIT_TRAIL WHERE AUDIT_TYPE='FineGrainedAudit' AND OBJECT_NAME='PHANCONG' ORDER BY EVENT_TIMESTAMP DESC;
